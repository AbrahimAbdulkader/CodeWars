...
public class DifferenceOfVolumesOfCuboid {
    ...
    public static int findDifference(final int[] firstCuboid, final int[] secondCuboid) {
        ...
        int firstCuboidSum = 1;
        int secondCuboidSum = 1;

        for(int i = 0; i < firstCuboid.length; i++) {
            firstCuboidSum *= firstCuboid[i];
        }
        for(int j = 0; j < secondCuboid.length; j++) {
            secondCuboidSum *= secondCuboid[j];
        }
        return Math.abs(firstCuboidSum - secondCuboidSum);
    }
